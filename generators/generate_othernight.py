...
FILE_TITLE_PT = 12  # Размер шрифта заголовка
SUBTITLE_TEXT = "Последующие ночи"  # Текст подзаголовка
SUBTITLE_PT = 14  # Размер шрифта подзаголовка
TITLE_TOP_OFFSET_MM = 4.0  # Отступ заголовка от верхнего края страницы

URL_IMAGE_SCALE = 0.625  # Масштаб картинок из интернета
ASSET_IMAGE_SCALE = 0.625  # Масштаб картинок из папки assets (Закат, Рассвет)
...
ROLE_PT = 13  # Размер шрифта названия роли
ABILITY_PT = 12  # Размер шрифта описания способности
...
def load_roles() -> List[Dict[str, Any]]:
    ...
    for i, r in enumerate(roles):
        ...
        enriched.append({
            ...
            "team": r.get("team"),
            "otherNight": float(fn),
            "otherNightReminder": r.get("otherNightReminder", ""),
            "image": r.get("image"),
            "order": i,
        })

    special_rows = [
        {"name": "Закат", "team": None, "otherNight": 0.0, "otherNightReminder": "Город засыпает.", "image": DUSK_IMAGE_PATH, "order": -1},
        {"name": "Рассвет", "team": None, "otherNight": 72.0, "otherNightReminder": "Город просыпается.", "image": DAWN_IMAGE_PATH, "order": 10**9},
    ]
...
OUTPUT_PDF = BASE_DIR / f"{safe_title}_othernight.pdf"
...
for role in roles:
    ...
    reminder_segments = segments_from_text(role.get("otherNightReminder",""))
    ...
    if role.get("image") in [str(DUSK_IMAGE_PATH), str(DAWN_IMAGE_PATH)]:
